...
Namespace PropertyPackages
    ...
    <System.Runtime.InteropServices.Guid(PCSAFTPropertyPackage.ClassId)> _
 <System.Serializable()> Public Class PCSAFTPropertyPackage

        Inherits PropertyPackages.PropertyPackage

        Public Shadows Const ClassId As String = "92A973A2-8970-4a17-A627-D78D21C533CE"

        Public MAT_KIJ(38, 38) As Object

        Private m_props As New PropertyPackages.Auxiliary.PROPS
        Public m_pr As New PropertyPackages.Auxiliary.PCSAFT
...
#Region "    DWSIM Functions"
        ...
        Public Overrides Function DW_CalcCp_ISOL(ByVal Phase1 As PropertyPackages.Phase, ByVal T As Double, ByVal P As Double) As Double
            Select Case Phase1
                Case Phase.Liquid
                    Return Me.m_pr.CalcCp("L", T, P, RET_VMOL(Phase1), RET_VKij(), RET_VMM, RET_VCAS, Me.AUX_CPm(Phase1, T))
                Case Phase.Aqueous
                    Return Me.m_pr.CalcCp("L", T, P, RET_VMOL(Phase1), RET_VKij(), RET_VMM, RET_VCAS, Me.AUX_CPm(Phase1, T))
                Case Phase.Liquid1
                    Return Me.m_pr.CalcCp("L", T, P, RET_VMOL(Phase1), RET_VKij(), RET_VMM, RET_VCAS, Me.AUX_CPm(Phase1, T))
                ...
                Case Phase.Liquid3
                    Return Me.m_pr.CalcCp("L", T, P, RET_VMOL(Phase1), RET_VKij(), RET_VMM, RET_VCAS, Me.AUX_CPm(Phase1, T))
                Case Phase.Vapor
                    Return Me.m_pr.CalcCp("V", T, P, RET_VMOL(Phase1), RET_VKij(), RET_VMM, RET_VCAS, Me.AUX_CPm(Phase1, T))
            End Select
        End Function
        ...
        Public Overrides Function DW_CalcEnergyFlowMistura_ISOL(ByVal T As Double, ByVal P As Double) As Double
            ...
            HL = Me.m_pr.CalcHRES("L", T, P, RET_VMOL(Phase.Liquid), RET_VKij(), RET_VMM, RET_VCAS, Me.RET_Hid(298.15, T, Phase.Liquid))
            HV = Me.m_pr.CalcHRES("V", T, P, RET_VMOL(Phase.Vapor), RET_VKij(), RET_VTC, RET_VCAS, Me.RET_Hid(298.15, T, Phase.Vapor))
        ...
        Public Overrides Sub DW_CalcProp(ByVal [property] As String, ByVal phase As Phase)
            ...
            Select Case [property].ToLower
                Case "compressibilityfactor"
                    result = Me.m_pr.CalcZ(RET_VMOL(phase), RET_VKij(), T, P, RET_VCAS, state)
                    Me.CurrentMaterialStream.Phases(phaseID).Properties.compressibilityFactor = result
                Case "heatcapacity", "heatcapacitycp"
                    result = Me.m_pr.CalcCp(state, T, P, RET_VMOL(phase), RET_VKij(), RET_VMM, RET_VCAS, Me.AUX_CPm(phase, T))
                    Me.CurrentMaterialStream.Phases(phaseID).Properties.heatCapacityCp = result
                Case "heatcapacitycv"
                    result = Me.m_pr.CalcCv(state, T, P, RET_VMOL(phase), RET_VKij(), RET_VMM, RET_VCAS, Me.AUX_CPm(phase, T))
                    Me.CurrentMaterialStream.Phases(phaseID).Properties.heatCapacityCv = result
                Case "enthalpy", "enthalpynf"
                    result = Me.m_pr.CalcHRES(state, T, P, RET_VMOL(phase), RET_VKij(), RET_VMM, RET_VCAS, Me.RET_Hid(298.15, T, phase))
                ...
                Case "entropy", "entropynf"
                    result = Me.m_pr.CalcSRES(state, T, P, RET_VMOL(phase), RET_VKij(), RET_VMM, RET_VCAS, Me.RET_Sid(298.15, T, P, phase))
                ...
                Case "excessenthalpy"
                    result = Me.m_pr.CalcHRES(state, T, P, RET_VMOL(phase), RET_VKij(), RET_VMM, RET_VCAS, 0)
                    Me.CurrentMaterialStream.Phases(phaseID).Properties.excessEnthalpy = result
                Case "excessentropy"
                    result = Me.m_pr.CalcSRES(state, T, P, RET_VMOL(phase), RET_VKij(), RET_VMM, RET_VCAS, 0)
        ...
        Public Overrides Sub DW_CalcPhaseProps(ByVal Phase As PropertyPackages.Phase)
            ...
            If phaseID = 3 Or phaseID = 4 Or phaseID = 5 Or phaseID = 6 Then
                ...
                If Convert.ToInt32(Me.Parameters("PP_USE_EOS_LIQDENS")) = 1 Then
                    Dim val As Double
                    val = m_pr.CalcZ(RET_VMOL(Phase), RET_VKij(), T, P, RET_VCAS, "L")
                    val = 1 / (8.314 * val * T / P)
                ...
                result = Me.m_pr.CalcHRES("L", T, P, RET_VMOL(dwpl), RET_VKij(), RET_VMM, RET_VCAS, Me.RET_Hid(298.15, T, dwpl))
                Me.CurrentMaterialStream.Phases(phaseID).Properties.enthalpy = result
                result = Me.m_pr.CalcSRES("L", T, P, RET_VMOL(dwpl), RET_VKij(), RET_VMM, RET_VCAS, Me.RET_Sid(298.15, T, P, dwpl))
                Me.CurrentMaterialStream.Phases(phaseID).Properties.entropy = result
                result = Me.m_pr.CalcZ(RET_VMOL(dwpl), RET_VKij(), T, P, RET_VCAS, "L")
                Me.CurrentMaterialStream.Phases(phaseID).Properties.compressibilityFactor = result
                result = Me.m_pr.CalcCp("L", T, P, RET_VMOL(dwpl), RET_VKij(), RET_VMM, RET_VCAS, Me.AUX_CPm(dwpl, T))
                Me.CurrentMaterialStream.Phases(phaseID).Properties.heatCapacityCp = result
                result = Me.m_pr.CalcCv("L", T, P, RET_VMOL(dwpl), RET_VKij(), RET_VMM, RET_VCAS, Me.AUX_CPm(dwpl, T))
                Me.CurrentMaterialStream.Phases(phaseID).Properties.heatCapacityCv = result
            ...
            ElseIf phaseID = 2 Then
                ...
                result = Me.AUX_VAPDENS(T, P)
                Me.CurrentMaterialStream.Phases(phaseID).Properties.density = result
                result = Me.m_pr.CalcHRES("V", T, P, RET_VMOL(Phase.Vapor), RET_VKij(), RET_VMM, RET_VCAS, Me.RET_Hid(298.15, T, Phase.Vapor))
                Me.CurrentMaterialStream.Phases(phaseID).Properties.enthalpy = result
                result = Me.m_pr.CalcSRES("V", T, P, RET_VMOL(Phase.Vapor), RET_VKij(), RET_VMM, RET_VCAS, Me.RET_Sid(298.15, T, P, Phase.Vapor))
                Me.CurrentMaterialStream.Phases(phaseID).Properties.entropy = result
                result = Me.m_pr.CalcZ(RET_VMOL(PropertyPackages.Phase.Vapor), RET_VKij(), T, P, RET_VCAS, "V")
                Me.CurrentMaterialStream.Phases(phaseID).Properties.compressibilityFactor = result
                result = Me.AUX_CPm(PropertyPackages.Phase.Vapor, T)
                result = Me.m_pr.CalcCp("V", T, P, RET_VMOL(dwpl), RET_VKij(), RET_VMM, RET_VCAS, Me.AUX_CPm(dwpl, T))
                Me.CurrentMaterialStream.Phases(phaseID).Properties.heatCapacityCp = result
                result = Me.m_pr.CalcCv("V", T, P, RET_VMOL(dwpl), RET_VKij(), RET_VMM, RET_VCAS, Me.AUX_CPm(dwpl, T))
                Me.CurrentMaterialStream.Phases(phaseID).Properties.heatCapacityCv = result
                result = Me.AUX_MMM(Phase)
                Me.CurrentMaterialStream.Phases(phaseID).Properties.molecularWeight = result
                result = Me.CurrentMaterialStream.Phases(phaseID).Properties.enthalpy.GetValueOrDefault * Me.CurrentMaterialStream.Phases(phaseID).Properties.molecularWeight.GetValueOrDefault
        ...
        Public Overrides Function RET_VKij() As Double(,)
            ...
            For Each cp As Interfaces.ICompound In Me.CurrentMaterialStream.Phases(0).Compounds.Values
                ...
                For Each cp2 As Interfaces.ICompound In Me.CurrentMaterialStream.Phases(0).Compounds.Values
                    val(i, l) = Me.RET_KIJ(cp.ConstantProperties.CAS_Number, cp2.ConstantProperties.CAS_Number)
                    l = l + 1
                Next
                i = i + 1
            Next
...
#End Region
        ...
        Public Overrides Function DW_CalcEnthalpy(ByVal Vx As System.Array, ByVal T As Double, ByVal P As Double, ByVal st As State) As Double
            ...
            If st = State.Liquid Then
                H = Me.m_pr.CalcHRES("L", T, P, Vx, RET_VKij(), RET_VMM, RET_VCAS, Me.RET_Hid(298.15, T, Vx))
            Else
                H = Me.m_pr.CalcHRES("V", T, P, Vx, RET_VKij(), RET_VMM, RET_VCAS, Me.RET_Hid(298.15, T, Vx))
            End If
        ...
        Public Overrides Function DW_CalcEnthalpyDeparture(ByVal Vx As System.Array, ByVal T As Double, ByVal P As Double, ByVal st As State) As Double
            ...
            If st = State.Liquid Then
                H = Me.m_pr.CalcHRES("L", T, P, Vx, RET_VKij(), RET_VMM, RET_VCAS, 0)
            Else
                H = Me.m_pr.CalcHRES("V", T, P, Vx, RET_VKij(), RET_VMM, RET_VCAS, 0)
            End If
        ...
        Public Overrides Function DW_CalcEntropy(ByVal Vx As System.Array, ByVal T As Double, ByVal P As Double, ByVal st As State) As Double
            ...
            If st = State.Liquid Then
                S = Me.m_pr.CalcSRES("L", T, P, Vx, RET_VKij(), RET_VMM, RET_VCAS, Me.RET_Sid(298.15, T, P, Vx))
            Else
                S = Me.m_pr.CalcSRES("V", T, P, Vx, RET_VKij(), RET_VMM, RET_VCAS, Me.RET_Sid(298.15, T, P, Vx))
            End If
        ...
        Public Overrides Function DW_CalcEntropyDeparture(ByVal Vx As System.Array, ByVal T As Double, ByVal P As Double, ByVal st As State) As Double
            ...
            If st = State.Liquid Then
                S = Me.m_pr.CalcSRES("L", T, P, Vx, RET_VKij(), RET_VMM, RET_VCAS, 0)
            Else
                S = Me.m_pr.CalcSRES("V", T, P, Vx, RET_VKij(), RET_VMM, RET_VCAS, 0)
            End If
        ...
        Public Overrides Function DW_CalcCv_ISOL(ByVal Phase1 As Phase, ByVal T As Double, ByVal P As Double) As Double
            Select Case Phase1
                Case Phase.Liquid
                    Return Me.m_pr.CalcCv("L", T, P, RET_VMOL(Phase1), RET_VKij(), RET_VMM, RET_VCAS, Me.AUX_CPm(Phase1, T))
                Case Phase.Aqueous
                    Return Me.m_pr.CalcCv("L", T, P, RET_VMOL(Phase1), RET_VKij(), RET_VMM, RET_VCAS, Me.AUX_CPm(Phase1, T))
                Case Phase.Liquid1
                    Return Me.m_pr.CalcCv("L", T, P, RET_VMOL(Phase1), RET_VKij(), RET_VMM, RET_VCAS, Me.AUX_CPm(Phase1, T))
                ...
                Case Phase.Liquid3
                    Return Me.m_pr.CalcCv("L", T, P, RET_VMOL(Phase1), RET_VKij(), RET_VMM, RET_VCAS, Me.AUX_CPm(Phase1, T))
                Case Phase.Vapor
                    Return Me.m_pr.CalcCv("V", T, P, RET_VMOL(Phase1), RET_VKij(), RET_VMM, RET_VCAS, Me.AUX_CPm(Phase1, T))
            End Select
        End Function
        ...
        Public Overrides Sub DW_CalcCompPartialVolume(ByVal phase As Phase, ByVal T As Double, ByVal P As Double)
            ...
            Select Case phase
                Case phase.Liquid
                    key = "1"
                    If Convert.ToInt32(Me.Parameters("PP_USE_EOS_LIQDENS")) = 1 Then
                        partvol = Me.m_pr.CalcPartialVolume(RET_VMOL(phase), RET_VKij(), T, P, RET_VCAS, "L")
                    Else
                ...
                Case phase.Liquid3
                    ...
                    End If
                Case phase.Vapor
                    partvol = Me.m_pr.CalcPartialVolume(RET_VMOL(phase), RET_VKij(), T, P, RET_VCAS, "V")
                    key = "2"
                Case PropertyPackages.Phase.Solid
        ...
        Public Overrides Function AUX_VAPDENS(ByVal T As Double, ByVal P As Double) As Double
            Dim val As Double
            Dim Z As Double = Me.m_pr.CalcZ(RET_VMOL(Phase.Vapor), RET_VKij, T, P, RET_VCAS, "V")
        ...
        Public Overrides Function DW_CalcFugCoeff(ByVal Vx As System.Array, ByVal T As Double, ByVal P As Double, ByVal st As State) As Double()
            ...
            If st = State.Liquid Then
                lnfug = m_pr.CalcLnFugCoeff(Vx, RET_VKij, T, P, RET_VCAS, "L")
            Else
                lnfug = m_pr.CalcLnFugCoeff(Vx, RET_VKij, T, P, RET_VCAS, "V")
            End If
