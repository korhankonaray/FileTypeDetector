...
CALL changelog_update("02/28/2018", "Adding handling to allow cases to be filtered by recently closed programs as well.", "Casey Love, Hennepin County")
...
'DIALOGS----------------------------------------------------------------------
BeginDialog case_transfer_dialog, 0, 0, 376, 390, "Select Parameters for Cases to Transfer"
  ...
  CheckBox 285, 250, 30, 10, "CASH", cash_in_past_checkbox
  EditBox 320, 245, 20, 15, cash_in_past_days
  CheckBox 285, 270, 30, 10, "SNAP", snap_in_past_checkbox
  EditBox 320, 265, 20, 15, snap_in_past_days
  CheckBox 285, 290, 25, 10, "HC", hc_in_past_checkbox
  EditBox 320, 285, 20, 15, hc_in_past_days
  CheckBox 285, 310, 25, 10, "GRH", grh_in_past_checkbox
  EditBox 320, 305, 20, 15, grh_in_past_days
  ...
  GroupBox 275, 195, 95, 140, "Program History"
  Text 280, 210, 90, 35, "Exclude cases in which the following programs have been active at any time in the time period specified."
  Text 345, 250, 20, 10, "days"
  Text 345, 270, 20, 10, "days"
  Text 345, 290, 20, 10, "days"
  Text 345, 310, 20, 10, "days"
EndDialog
...
Do
    ...
    If snap_in_past_checkbox = checked Then
        If snap_check = checked Then err_msg = err_msg & vbCr & "You cannot select active SNAP cases and exclude cases in which SNAP has been active recently."
        If snap_in_past_days = "" Then err_msg = err_msg & vbCr & "You must indicate how many days in the past you want the script to look for active SNAP."
    End If
    If cash_in_past_checkbox = checked Then
        If mfip_check = checked OR DWP_check = checked OR ga_check = checked OR msa_check = checked OR rca_check = checked Then err_msg = err_msg & vbCr & "You cannot select cases with a cash program active and exclude cases in which Cash has been active recently."
        If cash_in_past_days = "" Then err_msg = err_msg & vbCr & "You must indicate how many days in the past you want the script to look for ative Cash."
    End If
    If hc_in_past_checkbox = checked Then
        If HC_check = checked Then err_msg = err_msg & vbCr & "You cannot select cases with active HC and exlude cases in which HC has been active recently."
        If hc_in_past_days = "" Then err_msg = err_msg & vbCr & "You must indicate how many days in the past you want the script to look for active HC."
    End If
    If grh_in_past_checkbox = checked Then
        If GRH_check = checked Then err_msg = err_msg & vbCr & "You cannot select cases with active GRH and exlude cases in which GRH has been active recently."
        If grh_in_past_days = "" Then err_msg = err_msg & vbCr & "You must indicate how many days in the past you want the script to look for active GRH."
    End If
    IF err_msg <> "" THEN MsgBox "*** NOTICE!!! ***" & vbCr & err_msg & vbCr & vbCr & "Please resolve for the script to continue."
Loop until err_msg = ""
...
If cash_in_past_days <> "" Then cash_in_past_days = cash_in_past_days * 1
If snap_in_past_days <> "" Then snap_in_past_days = snap_in_past_days * 1
If grh_in_past_days  <> "" Then grh_in_past_days  = grh_in_past_days * 1
If hc_in_past_days   <> "" Then hc_in_past_days   = hc_in_past_days * 1
...
If snap_in_past_checkbox = checked OR query_all_check = checked Then
    ObjExcel.Cells(1, col_to_use).Value = "SNAP CLSD Eff"
    objExcel.Cells(1, col_to_use).Font.Bold = TRUE
    snap_in_past_col = col_to_use
    col_to_use = col_to_use + 1
    SNAP_in_past_letter_col = convert_digit_to_excel_column(snap_in_past_col)
End If
...
If cash_in_past_checkbox = checked OR query_all_check = checked Then
    ObjExcel.Cells(1, col_to_use).Value = "CASH CLSD Eff"
    objExcel.Cells(1, col_to_use).Font.Bold = TRUE
    cash_in_past_col = col_to_use
    col_to_use = col_to_use + 1
    cash_in_past_letter_col = convert_digit_to_excel_column(cash_in_past_col)
End If
...
If hc_in_past_checkbox = checked OR query_all_check = checked Then
    ObjExcel.Cells(1, col_to_use).Value = "HC CLSD Eff"
    objExcel.Cells(1, col_to_use).Font.Bold = TRUE
    hc_in_past_col = col_to_use
    col_to_use = col_to_use + 1
    hc_in_past_letter_col = convert_digit_to_excel_column(hc_in_past_col)
End If
...
If grh_in_past_checkbox = checked OR query_all_check = checked Then
    ObjExcel.Cells(1, col_to_use).Value = "GRH CLSD Eff"
    objExcel.Cells(1, col_to_use).Font.Bold = TRUE
    grh_in_past_col = col_to_use
    col_to_use = col_to_use + 1
    grh_in_past_letter_col = convert_digit_to_excel_column(grh_in_past_col)
End If
...
ReDim All_case_information_array (28, 0)
ReDim Full_case_list_array(16,0)
    ...
    If has_content_check <> " " then
        ...
        Do
            ...
            Do
                ...
                Redim Preserve Full_case_list_array (Ubound(Full_case_list_array,1), each_case) 'Resize the array for the next case

                Full_case_list_array(0,each_case) = MAXIS_case_number
                Full_case_list_array(1,each_case) = client_name
                Full_case_list_array(2,each_case) = replace(next_revw_date, " ", "/")
                Full_case_list_array(3,each_case) = cash_one_type
                Full_case_list_array(4,each_case) = cash_one_status
                Full_case_list_array(5,each_case) = cash_two_type
                Full_case_list_array(6,each_case) = cash_two_status
                Full_case_list_array(7,each_case) = SNAP_status
                Full_case_list_array(8,each_case) = HC_status
                Full_case_list_array(9,each_case) = EA_status
                Full_case_list_array(10,each_case) = GRH_status
                Full_case_list_array(11,each_case) = worker
                Full_case_list_array(12,each_case) = CCAP_status
...
'MsgBox Full_case_list_array(0, Ubound(Full_case_list_array, 2))
For n = 0 to Ubound(Full_case_list_array,2)	'This will check all the cases from REPT/ACTV for any of the criteria selected in initial dialog
    ...
    If snap_in_past_checkbox = checked OR cash_in_past_checkbox = checked OR hc_in_past_checkbox = checked OR grh_in_past_checkbox = checked OR query_all_check = checked Then
        ...
        mx_row = 8
        Do
            EMReadScreen prog_name, 4, mx_row, 4
            prog_name = trim(prog_name)
            Select Case prog_name

            Case "MFIP"
                If cash_in_past_checkbox = checked OR query_all_check = checked Then
                    If mfip_close_date = "" Then
                        EMReadScreen action_reason, 20, mx_row, 60
                        action_reason = trim(action_reason)
                        inelig_date = ""
                        If action_reason = "INELIGIBLE" Then EMReadScreen inelig_date, 8, mx_row, 18
                        If action_reason = "NO REVIEW - CLOSED" Then EMReadScreen inelig_date, 8, mx_row, 18
                        If action_reason = "CLOSED - NO 6 MO RPT" Then EMReadScreen inelig_date, 8, mx_row, 18
                        If action_reason = "NO HRF - CLOSED" Then EMReadScreen inelig_date, 8, mx_row, 18
                        mfip_close_date = inelig_date
                    End If
                End If

            Case "DWP"
                If cash_in_past_checkbox = checked OR query_all_check = checked Then
                    If dwp_close_date = "" Then
                        EMReadScreen action_reason, 20, mx_row, 60
                        action_reason = trim(action_reason)
                        inelig_date = ""
                        If action_reason = "INELIGIBLE" Then EMReadScreen inelig_date, 8, mx_row, 18
                        ...
                        If action_reason = "CLOSED - NO 6 MO RPT" Then EMReadScreen inelig_date, 8, mx_row, 18
                        If action_reason = "NO HRF - CLOSED" Then EMReadScreen inelig_date, 8, mx_row, 18
                        dwp_close_date = inelig_date
                    End If
                End If

            Case "GA"
                If cash_in_past_checkbox = checked OR query_all_check = checked Then
                    If ga_close_date = "" Then
                        EMReadScreen action_reason, 20, mx_row, 60
                        action_reason = trim(action_reason)
                        inelig_date = ""
                        If action_reason = "INELIGIBLE" Then EMReadScreen inelig_date, 8, mx_row, 18
                        ...
                        If action_reason = "CLOSED - NO 6 MO RPT" Then EMReadScreen inelig_date, 8, mx_row, 18
                        If action_reason = "NO HRF - CLOSED" Then EMReadScreen inelig_date, 8, mx_row, 18
                        ga_close_date = inelig_date
                    End If
                End If

            Case "MSA"
                If cash_in_past_checkbox = checked OR query_all_check = checked Then
                    If msa_close_date = "" Then
                        EMReadScreen action_reason, 20, mx_row, 60
                        action_reason = trim(action_reason)
                        inelig_date = ""
                        If action_reason = "INELIGIBLE" Then EMReadScreen inelig_date, 8, mx_row, 18
                        ...
                        If action_reason = "CLOSED - NO 6 MO RPT" Then EMReadScreen inelig_date, 8, mx_row, 18
                        If action_reason = "NO HRF - CLOSED" Then EMReadScreen inelig_date, 8, mx_row, 18
                        msa_close_date = inelig_date
                    End If
                End If

            Case "GRH"
                If grh_in_past_checkbox = checked OR query_all_check = checked Then
                    If grh_close_date = "" Then
                        EMReadScreen action_reason, 20, mx_row, 60
                        action_reason = trim(action_reason)
                        inelig_date = ""
                        If action_reason = "INELIGIBLE" Then EMReadScreen inelig_date, 8, mx_row, 18
                        ...
                        If action_reason = "CLOSED - NO 6 MO RPT" Then EMReadScreen inelig_date, 8, mx_row, 18
                        If action_reason = "NO HRF - CLOSED" Then EMReadScreen inelig_date, 8, mx_row, 18
                        grh_close_date = inelig_date
                    End If
                End If

            Case "FS"
                If snap_in_past_checkbox = checked OR query_all_check = checked Then
                    If snap_close_date = "" Then
                        EMReadScreen action_reason, 20, mx_row, 60
                        action_reason = trim(action_reason)
                        inelig_date = ""
                        If action_reason = "INELIGIBLE" Then EMReadScreen inelig_date, 8, mx_row, 18
                        ...
                        If action_reason = "CLOSED - NO 6 MO RPT" Then EMReadScreen inelig_date, 8, mx_row, 18
                        If action_reason = "NO HRF - CLOSED" Then EMReadScreen inelig_date, 8, mx_row, 18
                        snap_close_date = inelig_date
                    End If
                End If

            Case "MA"
                If hc_in_past_checkbox = checked OR query_all_check = checked Then
                    If ma_close_date = "" Then
                        EMReadScreen action_reason, 20, mx_row, 60
                        action_reason = trim(action_reason)
                        inelig_date = ""
                        If action_reason = "INELIGIBLE" Then EMReadScreen inelig_date, 8, mx_row, 18
            ...
            If mx_row = 18 Then
                PF8
                mx_row = 8
                EMReadScreen check_end, 9, 24, 14
                If check_end = "LAST PAGE" Then Exit Do
            End If

        Loop until prog_name = "AFDC"
        PF3
        PF3
        PF3


        If mfip_close_date = "" then mfip_close_date = "01/01/70"
        If dwp_close_date  = "" then dwp_close_date  = "01/01/70"
        If ga_close_date   = "" then ga_close_date   = "01/01/70"
        If msa_close_date  = "" then msa_close_date  = "01/01/70"

        cash_close_date = mfip_close_date
        If DateDiff("d", cash_close_date, dwp_close_date) >= 0 Then cash_close_date = dwp_close_date
        If DateDiff("d", cash_close_date, ga_close_date) >= 0  Then cash_close_date = ga_close_date
        If DateDiff("d", cash_close_date, msa_close_date) >= 0 Then cash_close_date = msa_close_date

        If snap_close_date = "" Then snap_close_date = "01/01/70"
        If grh_close_date  = "" Then grh_close_date  = "01/01/70"
        If grh_close_date  = "" Then grh_close_date  = "01/01/70"
        If ma_close_date   = "" Then ma_close_date   = "01/01/70"

        'MsgBox "MFIP " & mfip_close_date & vbcr & "Diff " & DateDiff("d", mfip_close_date, date)
        'MsgBox "DWP " & dwp_close_date & vbcr & "Diff " & DateDiff("d", dwp_close_date, date)
        'MsgBox "GA " & ga_close_date & vbcr & "Diff " & DateDiff("d", ga_close_date, date)
        'MsgBox "MSA " & msa_close_date & vbcr & "Diff " & DateDiff("d", msa_close_date, date)
        'MsgBox "SNAP " & snap_close_date & vbcr & "Diff " & DateDiff("d", snap_close_date, date)
        'MsgBox "GRH " & grh_close_date & vbcr & "Diff " & DateDiff("d", grh_close_date, date)
        'MsgBox " HC " & ma_close_date & vbcr & "Diff " & DateDiff("d", ma_close_date, date)

        Full_case_list_array(13, n) = cash_close_date
        Full_case_list_array(14, n) = snap_close_date
        Full_case_list_array(15, n) = grh_close_date
        Full_case_list_array(16, n) = ma_close_date
    ...
    Do	'The do loop is only here to be able to skip logic futher down in the list - it should never actually loop
        ...
        End If
        If cash_in_past_checkbox = checked Then
            cash_difference = DateDiff("d", Full_case_list_array(13,n), date)
            If cash_difference < cash_in_past_days Then Save_case_for_transfer = FALSE
    ''		MsgBox "Cash Date - " & Full_case_list_array(13, n) & vbCr & "Today " & date & vbCr & "DAYS " & cash_in_past_days & vbCr & "Diff: " & cash_difference & vbCr & Save_case_for_transfer
        End If
        If snap_in_past_checkbox = checked Then
            snap_difference = DateDiff("d", Full_case_list_array(14,n), date)
            If snap_difference < snap_in_past_days Then Save_case_for_transfer = FALSE
        End If
        If grh_in_past_checkbox = checked Then
            grh_difference = DateDiff("d", Full_case_list_array(15,n), date)
            If grh_difference < grh_in_past_days Then Save_case_for_transfer = FALSE
        End if
        If hc_in_past_checkbox = checked Then
            hc_difference = DateDiff("d", Full_case_list_array(16,n), date)
            If hc_difference < hc_in_past_days Then Save_case_for_transfer = FALSE
        End If
    ...
    IF Save_case_for_transfer = TRUE then
        ...
        If Full_case_list_array(13,n) = "01/01/70" Then Full_case_list_array(13,n) = " - "
        If Full_case_list_array(14,n) = "01/01/70" Then Full_case_list_array(14,n) = " - "
        If Full_case_list_array(15,n) = "01/01/70" Then Full_case_list_array(15,n) = " - "
        If Full_case_list_array(16,n) = "01/01/70" Then Full_case_list_array(16,n) = " - "
        All_case_information_array(25,k) = Full_case_list_array(13,n)
        All_case_information_array(26,k) = Full_case_list_array(14,n)
        All_case_information_array(27,k) = Full_case_list_array(15,n)
        All_case_information_array(28,k) = Full_case_list_array(16,n)
        ...
        IF cash_in_past_checkbox = checked OR query_all_check = checked THEN ObjExcel.Cells(excel_row, cash_in_past_col).Value = All_case_information_array(25,k)
        IF snap_in_past_checkbox = checked OR query_all_check = checked THEN ObjExcel.Cells(excel_row, snap_in_past_col).Value = All_case_information_array(26,k)
        IF grh_in_past_checkbox = checked OR query_all_check = checked THEN ObjExcel.Cells(excel_row, grh_in_past_col).Value = All_case_information_array(27,k)
        IF hc_in_past_checkbox = checked OR query_all_check = checked THEN ObjExcel.Cells(excel_row, hc_in_past_col).Value = All_case_information_array(28,k)
