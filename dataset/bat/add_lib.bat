...
REM Query the host for connected Android devices. If one is found, install library into the /system/lib folder of the device.
...
echo Installing !BINARY_PATH! on !chosendevice! at /system/lib/!FILENAME!
...
adb -s "!chosendevice!" shell "su -c 'cp /sdcard/!FILENAME! /system/lib/!FILENAME!'"
echo Delete copy of !FILENAME! from sdcard
adb -s "!chosendevice!" shell "su -c 'rm /sdcard/!FILENAME!'"
echo Grant permissions for !FILENAME! library
adb -s "!chosendevice!" shell "su -c 'chmod 0644 /system/lib/!FILENAME!'"
echo Finished
timeout 2
goto :cleanup